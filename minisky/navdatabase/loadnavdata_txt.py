...
def loadnavdata_txt():
    ...
    with open(minisky.data(settings.navdata_path) / "nav.dat", "rb") as f:
        ...
        for line in f:
            ...
            if wptype == "NDB":
                ...
                wptdata["wpfreq"].append(float(fields[4]) / 100.0)  # VOR freq in MHz
            else:
                wptdata["wpfreq"].append(0.0)

            if wptype in ["VOR", "NDB"]:
                wptdata["wpvar"].append(
                    float(fields[6])
                )  # Magnetic variation in np.degrees
    ...
    # ----------  Read  fix.dat file ----------
    with open(minisky.data(settings.navdata_path) / "fix.dat", "rb") as f:
    ...
    with open(minisky.data(settings.navdata_path) / "awy.dat", "rb") as f:
    ...
    with open(minisky.data(settings.navdata_path) / "airports.dat", "rb") as f:
        types = {"L": 1, "M": 2, "S": 3}
        for line in f:
            line = line.decode(encoding="ascii", errors="ignore").strip()
            # Skip empty lines or comments
            ...
            aptdata["apid"].append(
                fields[0].strip()
            )  # id, no leading or trailing spaces
            aptdata["apname"].append(
                fields[1].strip()
            )  # name, no leading or trailing spaces

            aptdata["aplat"].append(float(fields[2]))  # latitude [deg]
            aptdata["aplon"].append(float(fields[3]))  # longitude [deg]

            aptdata["aptype"].append(
                types[fields[4].strip()[0]]
    ...
    # Get fir names
    for filname in (minisky.data(settings.navdata_path) / "fir").iterdir():
        if filname.suffix == ".txt":
            ...
            with open(filname, "rb") as f:
                for line in f:
                    ...
                    lon = lonsign * londeg + lonmin / 60.0 + lonsec / 3600.0
    ...
    with open(minisky.data(settings.navdata_path) / "icao-countries.dat", "rb") as f:
        for line in f:
            line = line.decode(encoding="ascii", errors="ignore").strip()
            # Skip empty lines or comments
            if len(line) == 0 or line[0] == "#":
            ...
            codata["coname"].append(
                fields[0].strip()
            )  # id, no leading or trailing spaces
            codata["cocode2"].append(
                fields[1].strip().upper()
            )  # name, no leading or trailing spaces
...
def loadthresholds_txt():
    ...
    rwythresholds = dict()
    curthresholds = None
    zfile = ZipFile(minisky.data(settings.navdata_path) / "apt.zip")
...
def thresholds(lat1, lon1, lat2, lon2, offset):
    ...
    bearing = np.arctan2(
        np.sin(deltal) * np.cos(lat2),
        (np.cos(lat1) * np.sin(lat2) - np.sin(lat1) * np.cos(lat2) * np.cos(deltal)),
    )
...
def thrpoints(lat1, lon1, d, bearing):
    ...
    http://www.movable-type.co.uk/scripts/latlong.html"""
    latthres = np.arcsin(
        np.sin(lat1) * np.cos(d) + np.cos(lat1) * np.sin(d) * np.cos(bearing)
    )

    lonthres = lon1 + np.arctan2(
