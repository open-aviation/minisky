...
from minisky.tools.misc import tim2txt
...
class metric_Area:
    ...
    def cellArea(self):
        ...
        point2 = [
            self.regions[self.ncells * self.nlevels - 1, 0],
            self.regions[self.ncells * self.nlevels - 1, 1],
        ]
        point3 = [
            self.regions[(self.ncells - 1) * self.ncells * self.nlevels, 0],
            self.regions[(self.ncells - 1) * self.ncells * self.nlevels, 1],
        ]
        point4 = [
            self.regions[self.ncells * self.ncells * self.nlevels - 1, 0],
            self.regions[self.ncells * self.ncells * self.nlevels - 1, 1],
        ]
    ...
    def FIR_circle(self, fir_number):
        ...
        fir_lat.append(minisky.navdb.fir[fir_number][1])
        fir_lon.append(minisky.navdb.fir[fir_number][2])
...
class metric_CoCa:
    def __init__(self, regions):
        ...
        self.numberofcells = (
            self.region.ncells * self.region.ncells * self.region.nlevels
        )
    ...
    def findCell(self, cells, lat, lon, fl):
        ...
        if i > -1:
            for j in range(
                0, self.region.ncells * self.region.nlevels, self.region.nlevels
            ):
                if (
                    cells[i + j, 1] > lon
                    and lon < (cells[-1, 1] + 0.6)
                    and lon > cells[0, 1]
                ):
            ...
            if j > -1:
                for k in range(0, self.region.nlevels, 1):
                    if (
                        cells[i + j + k, 2] > fl
                        and fl < (cells[-1, 2] + self.region.deltaFL)
                        and fl > cells[0, 2]
                    ):
                        k = k - 1
                        break
    ...
    def cellPlot(self):
        cell = [floor(x / 12) for x in minisky.traf.cell]
        ...
        for i in range(0, self.numberofcells):
            ...
            if len(times) > 1:
                for k in range(0, len(times)):
                    ...
                    acinteractions.append(
                        aircraft * (aircraft - 1) * (actime**aircraft)
                    )
                ...
                self.cocametric[name][l][1] = (
                    self.precocametric[name][l][1] / self.precocametric[name][l][0]
                )
                self.cocametric[name][l][2] = (
                    self.precocametric[name][l][2] / self.precocametric[name][l][0]
                )
                self.cocametric[name][l][3] = (
                    self.precocametric[name][l][3] / self.precocametric[name][l][0]
                )
                self.cocametric[name][l][4] = (
                    self.precocametric[name][l][4] / self.precocametric[name][l][0]
                )
                self.cocametric[name][l][0] = self.cocametric[name][l][1] * (
                    self.cocametric[name][l][2]
                    + self.cocametric[name][l][3]
                    + self.cocametric[name][l][4]
                )
    ...
    def AircraftCell(self, cells, time):
        if floor(time) >= self.resettime:
            ...
            # filedata = minisky.data(settings.log_path) / "coca_20120727-78am-1hour.npy"
            # self.cellPlot(traf)
        ...
        for i in range(minisky.traf.ntraf):
            lat = minisky.traf.lat[i]
            lon = minisky.traf.lon[i]
            fl = minisky.traf.alt[i] / ft
            cellN = self.findCell(cells, lat, lon, fl)

            if cellN > 0:
                minisky.traf.cell = np.append(minisky.traf.cell, cellN)
                name = "cell" + str(cellN)

                index = np.where(minisky.traf.id[i] == self.cells[name][:, [0]])[0]
                if len(index) != 1:
                    j = 0
                    for j in range(0, len(self.cells[name])):
                        if self.cells[name][j][0] == "":
                            break
                    self.cells[name][j][0] = minisky.traf.id[i]
                    self.cells[name][j][1] = time
                    self.cells[name][j][2] = minisky.traf.ahdg[i]
                    self.cells[name][j][3] = (
                        eas2tas(minisky.traf.selspd[i], minisky.traf.selalt[i]) / kts
                    )
                    self.cells[name][j][4] = minisky.traf.selvs[i] / fpm
                    self.cells[name][j][5] = time
                if len(index) == 1:
                    createtime = float(self.cells[name][index[0]][5])
                    self.cells[name][index[0]][1] = str(time - createtime)

                # self.newaircraft['callsign'][i] = minisky.traf.id[i]
...
class metric_HB:
    ...
    def selectTraffic(self):
        ...
        # RECTANGLE AREA
        # for i in range(0,minisky.traf.ntraf):
        #     if nx.pnpoly(minisky.traf.lat[i],minisky.traf.lon[i],self.selected_area) == 1:
        #         traf_selected_lat = np.append(traf_selected_lat,minisky.traf.lat[i])
        #         traf_selected_lon = np.append(traf_selected_lon,minisky.traf.lon[i])
        #         traf_selected_alt = np.append(traf_selected_alt,minisky.traf.alt[i])
        #         traf_selected_tas = np.append(traf_selected_tas,minisky.traf.tas[i])
        #         traf_selected_trk = np.append( traf_selected_trk,minisky.traf.trk[i])
        #         traf_selected_ntraf = traf_selected_ntraf + 1

        # CIRCLE AREA (FIR Circle)
        for i in range(0, minisky.traf.ntraf):
            dist = latlondist(
                minisky.sim.metric.fir_circle_point[0],
                minisky.sim.metric.fir_circle_point[1],
                minisky.traf.lat[i],
                minisky.traf.lon[i],
            )

            if dist / nm < minisky.sim.metric.fir_circle_radius:
                traf_selected_lat = np.append(traf_selected_lat, minisky.traf.lat[i])
                traf_selected_lon = np.append(traf_selected_lon, minisky.traf.lon[i])
                traf_selected_alt = np.append(traf_selected_alt, minisky.traf.alt[i])
                traf_selected_tas = np.append(traf_selected_tas, minisky.traf.tas[i])
    ...
    def applymetric(self):
        ...
        [self.rel_trk, self.pos] = geo.qdrdist_matrix(
            self.initiallat,
            self.initiallon,
    ...
    def apply_twoCircleMethod(self):
        ...
        [H0, S0] = geo.qdrdist_matrix(
            np.mat(self.lat), np.mat(self.lon), np.mat(self.lat), np.mat(self.lon)
        )
        ...
        ha_new11, ha_new21, ha_new12, ha_new22, t1d1, t1d2, t2d1, t2d2 = (
            self.calc_angles(Vb, Hb, VaVa, H0, arcsin, S0)
        )
        ...
        ha_1, ha_2, ha_3, ha_4, t1, t2, t3, t4 = self.conditions(
            ha_1, ha_2, ha_3, ha_4, t1, t2, t3, t4, Va, Vb, Ha, Hb
        )
        ...
        for k in range(0, self.ntraf):
            ...
            if len(ac_angles[str(k)][0]) > 0:
                for z in range(0, len(ac_angles[str(k)])):
                    ...
                    if (
                        ac_angles_st180 < ac_angle180min
                        and ac_angles_en180 < ac_angle180min
                    ) or (
                        ac_angles_st360 < ac_angle360min
                        and ac_angles_en360 < ac_angle360min
                    ):
                    ...
                    if ac_angles[str(k)][z][-1] < 90 and ac_angles[str(k)][z][-1] > -90:
                        ac_score[str(k)] = (
                            ac_score[str(k)]
                            + (ac_angles[str(k)][z][-1] - ac_angles[str(k)][z][0]) / 180
                        )
                    else:
                        ac_score[str(k)] = (
                            ac_score[str(k)]
                            + (ac_angles[str(k)][z][-1] + 360 - ac_angles[str(k)][z][0])
                            / 180
                        )
    ...
    def saveData(self):
        ...
        fname = minisky.data(settings.log_path) / "Metric-HB" / f"{step}-BlueSky.csv"
    ...
    def complexity_plot(self):
        ...
        self.plot_complexity.set_ydata(
            np.append(self.plot_complexity.get_ydata(), self.complexity[self.step][0])
        )
        self.plot_complexity2.set_xdata(np.append(self.plot_complexity2.get_xdata(), t))
        self.plot_complexity2.set_ydata(
            np.append(self.plot_complexity2.get_ydata(), self.complexity[self.step][1])
        )
    ...
    def apply_heading_range(self):
        ...
        angle_rel = np.arctan2(
            np.subtract(va_sxa.T, vb_sxb), np.subtract(va_cxa.T, vb_cxb)
        )
...
class Metric:
    ...
    def toggle(self, flag, dt=None):
        ...
        if type(flag) == bool and not flag:
            ...
            if not minisky.traf.area == "Circle":
            ...
            return True, "Activated %s (%d), dt=%.2f" % (
                self.name[self.metric_number],
                flag,
                self.dt,
            )
    ...
    def update(self):
        ...
        if self.metric_number == -1 or minisky.traf.ntraf < 1:
            return

        """Update: to call for regular logging & runtime analysis"""
        # Only do something when time is there
        if abs(minisky.sim.simt - self.t0) < self.dt:
            return
        self.t0 = minisky.sim.simt  # Update time for scheduler
        if self.metricstime == 0:
            self.tbegin = minisky.sim.simt
            self.metricstime = 1
            print("METRICS STARTED")
            # FIR_circle(minisky.navdb,self.fir_number)
        ...
        if minisky.sim.simt >= 0:
            if self.metric_number == 0:
                self.metric[self.metric_number].AircraftCell(
                    self.cells, minisky.sim.t - self.tbegin
                )
            elif self.metric_number == 1:
                self.metric[self.metric_number].applymetric()

        print(
            "Number of Aircraft in Research Area (FIR):"
            + str(self.metric[self.metric_number].ntraf)
        )

        deleteAC = []
        for i in range(0, minisky.traf.ntraf):
            if (
                minisky.traf.selvs[i] <= 0
                and (minisky.traf.selalt[i] / ft) < 750
                and minisky.traf.selspd[i] < 300
            ):
                deleteAC.append(minisky.traf.id[i])

            elif minisky.traf.selvs[i] <= 0 and (minisky.traf.selalt[i] / ft) < 10.0:
                deleteAC.append(minisky.traf.id[i])

            if minisky.traf.selvs[i] <= 0 and minisky.traf.selspd[i] < 10.0:
                deleteAC.append(minisky.traf.id[i])

        for i in range(0, len(deleteAC)):
            minisky.traf.delete(deleteAC[i])

        # Heartbeat for test
        self.write(minisky.sim.simt, "NTRAF;" + str(minisky.traf.ntraf))
        return
