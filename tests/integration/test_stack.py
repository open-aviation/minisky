...
class TestReadscn:
    ...
    def test_blank_and_comment_lines_skipped(self, runtime):
...
class TestHelp:
    def test_help_writes_command_reference(self, runtime, sim, tmp_path, monkeypatch):
...
class TestVarExplorer:
    def test_variable_get_without_index(self, runtime, run_cmd):
        run_cmd("CRE KL204,B744,52,4,45,FL250,350")
    ...
    def test_variable_get_with_index(self, runtime, run_cmd):
        run_cmd("CRE KL204,B744,52,4,45,FL250,350")
...
class TestSynonyms:
    def test_airway_synonyms_point_to_pos(self, runtime):
...
class TestErrors:
    def test_unknown_command_echoes_error(self, runtime, run_cmd):
        output = run_cmd("BOGUSCMD 42")
        assert "unknown command" in output.lower()

    def test_command_on_missing_aircraft_reports_error(self, runtime, run_cmd):
        output = run_cmd("ALT NOSUCH FL100")
        assert output  # some error text is echoed
        assert runtime.traffic.ntraf == 0

    def test_sim_survives_bad_command(self, runtime, run_cmd):
...
class TestArgumentSpecs:
    def test_all_registered_specs_resolve_to_parsers(self, runtime):
