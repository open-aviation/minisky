...
interface between the per-aircraft [`Route`][minisky.Route]
objects (event-driven, scalar waypoint switching) and the vectorized
LNAV/VNAV guidance in [`Autopilot`][minisky.Autopilot].
...
from minisky.aero import g0, vcas2tas
...
class ActiveWaypoint(TrafficArrays):
    ...
    waypoint switching (see [`Autopilot.wppassingcheck`][minisky.Autopilot.wppassingcheck]
    and `route.direct`), so continuous guidance
