...
from clearsky.tools.misc import (
    txt2alt,
    txt2bool,
    txt2hdg,
    txt2lat,
...
from clearsky.tools.position import Position, islat
...
class AcidArg(Parser):
    ...
    def parse(self, argstring):
        ...
        acid = arg.upper()
        if acid in cs.traf.groups:
            idx = cs.traf.groups.listgroup(acid)
        else:
            idx = cs.traf.id2idx(acid)
            if idx < 0:
                raise ArgumentError(f"Aircraft with callsign {acid} not found")

            # Update ref position for navdb lookup
            refdata.lat = cs.traf.lat[idx]
            refdata.lon = cs.traf.lon[idx]
            refdata.acidx = idx
...
class WpinrouteArg(Parser):
    ...
    def parse(self, argstring):
        ...
        if (
            refdata.acidx >= 0
            and wpname in cs.traf.ap.route[refdata.acidx].wpname
            or wpname == "*"
        ):
            return wpname, argstring
        raise ArgumentError(
            f"{wpname} not found in the route of {cs.traf.id[refdata.acidx]}"
        )
...
class WptArg(Parser):
    ...
    def parse(self, argstring):
        ...
        idx = cs.traf.id2idx(name)
        if idx >= 0:
            name = f"{cs.traf.lat[idx]},{cs.traf.lon[idx]}"
        ...
        elif argstring[:2].upper() == "RW" and name in cs.navdb.aptid:
            arg, argstring = re_getarg.match(argstring).groups()
            name = name + "/" + arg.upper()
...
class PosArg(Parser):
    ...
    def parse(self, argstring):
        ...
        # Try aircraft first: translate a/c id into a valid position text with a lat,lon
        idx = cs.traf.id2idx(argu)
        if idx >= 0:
            return cs.traf.lat[idx], cs.traf.lon[idx], argstring
        ...
        # apt,runway ? Combine into one string with a slash as separator
        if argstring[:2].upper() == "RW" and argu in cs.navdb.aptid:
            arg, argstring = re_getarg.match(argstring).groups()
            argu = argu + "/" + arg.upper()

        if refdata.lat is None:
            refdata.lat, refdata.lon = cs.scr.getviewctr()
