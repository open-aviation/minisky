...
from clearsky.stack.cmdparser import command, commandgroup
from clearsky.tools.aero import casormach2tas, fpm, ft, kts, tas2cas
...
def saveic(filename: "word" = ""):
    ...
    if not filename.is_absolute():
        filename = cs.data(cs.settings.scenario_path) / filename
    ...
    saveict0 = cs.sim.simt

    for i in range(cs.traf.ntraf):
        # CRE acid,type,lat,lon,hdg,alt,spd
        cmdline = (
            f"CRE {cs.traf.id[i]},{cs.traf.type[i]},{cs.traf.lat[i]},"
            f"{cs.traf.lon[i]},{cs.traf.trk[i]},{cs.traf.alt[i] / ft},"
            f"{tas2cas(cs.traf.tas[i], cs.traf.alt[i]) / kts}"
        )
        ...
        # Autopilot commands
        # Altitude & VS
        if abs(cs.traf.alt[i] - cs.traf.ap.alt[i]) > 10.0:
            cmdline = "ALT " + cs.traf.id[i] + "," + repr(cs.traf.ap.alt[i] / ft)
            f.write(timtxt + cmdline + "\n")

            # VS acid,vs
            if abs(cs.traf.vs[i]) > 0.05:  # 10 fpm dead band
                if abs(cs.traf.ap.vs[i]) > 0.05:
                    vs_ = cs.traf.ap.vs[i] / fpm
                else:
                    vs_ = cs.traf.vs[i] / fpm

                cmdline = f"VS {cs.traf.id[i]},{vs_}"
                f.write(timtxt + cmdline + "\n")

        # Heading as well when heading select
        delhdg = (cs.traf.hdg[i] - cs.traf.ap.trk[i] + 180.0) % 360.0 - 180.0
        if abs(delhdg) > 0.5:
            cmdline = "HDG " + cs.traf.id[i] + "," + repr(cs.traf.ap.trk[i])
            f.write(timtxt + cmdline + "\n")

        # Speed select mode? => Record a speed command
        aptas = casormach2tas(cs.traf.selspd[i], cs.traf.ap.spd[i])
        delspd = aptas - cs.traf.tas[i]

        if (
            abs(delspd) > 0.5 * kts
        ):  # difference equal more than 1 knot (rounded), so 0.5* 0.514444 m/s
            if not (0.001 < cs.traf.selspd[i] < 1.0):  # Check for Mach
                spdcmd = round(cs.traf.selspd[i] / kts, 2)
            else:
                spdcmd = cs.traf.selspd[i]

            cmdline = "SPD " + cs.traf.id[i] + "," + repr(spdcmd)
            f.write(timtxt + cmdline + "\n")

        # DEST acid,dest-apt
        if cs.traf.ap.dest[i] != "":
            cmdline = "DEST " + cs.traf.id[i] + "," + cs.traf.ap.dest[i]
            f.write(timtxt + cmdline + "\n")

        # ORIG acid,orig-apt
        if cs.traf.ap.orig[i] != "":
            cmdline = "ORIG " + cs.traf.id[i] + "," + cs.traf.ap.orig[i]
            f.write(timtxt + cmdline + "\n")

        # Route with ADDWPT
        route = cs.traf.ap.route[i]
        for iwp in range(route.nwp):
            # dets and orig al already done, skip them here
            if iwp == 0 and route.wpname[iwp] == cs.traf.ap.orig[i]:
                continue

            if iwp == route.nwp - 1 and route.wpname[iwp] == cs.traf.ap.dest[i]:
                continue

            # add other waypoints
            cmdline = "ADDWPT " + cs.traf.id[i] + " "
            wpname = route.wpname[iwp]
            if wpname[: len(cs.traf.id[i])] == cs.traf.id[i]:
...
def savecmd(cmd, line):
    ...
    timtxt = tim2txt(cs.sim.simt - saveict0)
