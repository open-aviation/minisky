...
from clearsky.stack import argparser, commands
from clearsky.stack.argparser import ArgumentError, Parameter, getnextarg
...
def delete_element(*arg):
    if isinstance(arg[0], str) and arg[0] == "WIND":
        return cs.traf.wind.clear()
    elif isinstance(arg[0], str):
        return cs.tools.areafilter.deleteArea(arg[0])
    elif hasattr(arg[0], "groupname"):
        return cs.traf.groups.delgroup(arg[0])
    else:
        return cs.traf.delete(arg)
...
def checkscen():
    ...
    if Stack.scencmd:
        # Find index of first timestamp exceeding cs.sim.simt
        idx = next((i for i, t in enumerate(Stack.scentime) if t > cs.sim.simt), None)
