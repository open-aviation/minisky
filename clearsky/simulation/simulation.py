...
class Simulation:
    ...
    def step(self, dt_increment=0):
        ...
        real-time rate.
        """
        if self.state == cs.INIT:
            # Simulation starts as soon as there is traffic, or pending commands
            if cs.traf.ntraf > 0 or len(cs.stack.get_scendata()[0]) > 0:
                self.op()
    ...
    def update(self):
        ...
        if (not self.ffmode or self.state != cs.OP) and remainder > MINSLEEP:
        ...
        if self.ffstop is not None and self.simt >= self.ffstop:
            if self.benchdt > 0.0:
                cs.scr.echo(
                    ...
                    % (cs.scr.samplecount, time.time() - self.bencht)
                )
                self.benchdt = -1.0
                self.hold()
            else:
    ...
    def op(self):
        ...
        self.state = cs.OP
        self.set_dtmult(1.0)
        cs.scr.echo("Simulation running in real-time mode")
    ...
    def fastforward(self, nsec=None):
        ...
        cs.scr.echo("Entering fast-time mode")
