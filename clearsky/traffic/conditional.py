...
from clearsky.tools.geo import qdrdist
...
class Condition:
    ...
    def update(self):
        ...
        acidxlst = np.array(cs.traf.id2idx(self.id))
        if len(acidxlst) > 0:
        ...
        for j in range(self.ncond):
            if self.condtype[j] == postype:
                qdr, dist = qdrdist(
                    cs.traf.lat[acidxlst[j]],
                    cs.traf.lon[acidxlst[j]],
                    self.posdata[j][0],
                    self.posdata[j][1],
                )
                actdist[j] = dist  # [nm]
        ...
        self.actual = (
            (self.condtype == alttype) * cs.traf.alt[acidxlst]
            + (self.condtype == spdtype) * cs.traf.cas[acidxlst]
    ...
    def addcondition(self, acidx, icondtype, target, actual, cmdtxt, latlon=None):
        ...
        self.id.append(cs.traf.id[acidx])
