...
class ConflictResolution(Entity, replaceable=True):
    ...
    def __init__(self):
        ...
        self.resofach = cs.settings.asas_marh
        self.resofacv = cs.settings.asas_marv
    ...
    def resumenav(self, conf, ownship, intruder):
        ...
        for conflict in self.resopairs:
            idx1, idx2 = cs.traf.id2idx(conflict)
        ...
        for idx, active in changeactive.items():
            ...
            if not active:
                ...
                iwpid = cs.traf.ap.route[idx].findact(idx)
                if iwpid != -1:  # To avoid problems if there are no waypoints
                    cs.traf.ap.route[idx].direct(
                        idx, cs.traf.ap.route[idx].wpname[iwpid]
                    )
    ...
    def setnoreso(self, *idx: "acid"):
        ...
        if not idx:
            return (
                ...
                + ", ".join(np.array(cs.traf.id)[self.noresoac]),
            )
        idx = list(idx)
    ...
    def setresooff(self, *idx: "acid"):
        ...
        if not idx:
            return (
                ...
                + ", ".join(np.array(cs.traf.id)[self.resooffac]),
            )
        else:
            idx = list(idx)
