...
from clearsky.tools.misc import latlon2txt
from clearsky.traffic.asas import ConflictDetection, ConflictResolution
...
class Traffic(Entity):
    ...
    def __create_aircraft(
        ...
        for j in range(self.ntraf - n, self.ntraf):
            for cmdtxt in self.crecmdlist:
                cs.stack.stack(self.id[j] + " " + cmdtxt)
    ...
    def creconfs(
        ...
        cpa = dcpa * nm
        pzr = cs.settings.asas_pzr * nm
        pzh = cs.settings.asas_pzh * ft
    ...
    @timed_function(name="asas", dt=cs.settings.asas_dt, manual=True)
    ...
    def update_airspeed(self):
        ...
        need_ax = np.abs(delta_spd) > np.abs(cs.sim.simdt * self.perf.axmax)
        self.ax = need_ax * np.sign(delta_spd) * self.perf.axmax
        # Update velocities
        self.tas = np.where(
            need_ax, self.tas + self.ax * cs.sim.simdt, self.aporasas.tas
        )
        ...
        self.swhdgsel = np.abs(delhdg) > np.abs(cs.sim.simdt * turnrate)

        # Update heading
        self.hdg = (
            np.where(
                self.swhdgsel,
                self.hdg + cs.sim.simdt * turnrate * np.sign(delhdg),
                self.aporasas.hdg,
            )
            % 360.0
        )
        ...
        #            10 * ft, np.abs(2 * cs.sim.simdt * self.vs))
        ...
        self.swaltsel = np.abs(delta_alt) > 1.05 * np.maximum(
            np.abs(cs.sim.simdt * self.aporasas.vs), np.abs(cs.sim.simdt * self.vs)
        )
        ...
        self.vs = np.where(need_az, self.vs + self.az * cs.sim.simdt, target_vs)
    ...
    def update_pos(self):
        ...
        self.alt = np.where(
            self.swaltsel,
            np.round(self.alt + self.vs * cs.sim.simdt, 6),
            self.aporasas.alt,
        )
        self.lat = self.lat + np.degrees(cs.sim.simdt * self.gsnorth / Rearth)
        self.coslat = np.cos(np.deg2rad(self.lat))
        self.lon = self.lon + np.degrees(
            cs.sim.simdt * self.gseast / self.coslat / Rearth
        )
        self.distflown += self.gs * cs.sim.simdt
    ...
    def position_by_name(self, name: str):
        ...
        idx_airport = cs.navdb.getaptidx(name)
        if idx_airport >= 0:
            airport_sizes = ["large", "medium", "small"]
            airport_size = airport_sizes[max(-1, cs.navdb.aptype[idx_airport] - 1)]

            aptname = cs.navdb.aptname[idx_airport]
            aptlat = cs.navdb.aptlat[idx_airport]
            aptlon = cs.navdb.aptlon[idx_airport]
            aptelev = cs.navdb.aptelev[idx_airport]

            # country informatation
            idx_cc = cs.navdb.cocode2.index(cs.navdb.aptco[idx_airport].upper())
            country_name = cs.navdb.coname[idx_cc].upper()
            country_code = cs.navdb.aptco[idx_airport]
            ...
            if cs.navdb.aptid[idx_airport] in cs.navdb.rwythresholds:
                runways = cs.navdb.rwythresholds[cs.navdb.aptid[idx_airport]].keys()
                if runways:
        ...
        # Not found as airport, try waypoints & navaids
            ...
            idx_waypoints = cs.navdb.getwpindices(name)
            if idx_waypoints[0] >= 0:
                ...
                for i in idx_waypoints:
                    # One line type text
                    if typetxt == "":
                        typetxt = typetxt + cs.navdb.wptype[i]
                    else:
                        typetxt = typetxt + " and " + cs.navdb.wptype[i]

                    # Description: multi-line
                    samedesc = cs.navdb.wpdesc[i] == lastdesc
                    if desctxt == "":
                        desctxt = desctxt + cs.navdb.wpdesc[i]
                        lastdesc = cs.navdb.wpdesc[i]
                    elif not samedesc:
                        desctxt = desctxt + "\n" + cs.navdb.wpdesc[i]
                        lastdesc = cs.navdb.wpdesc[i]

                    # Navaid: frequency
                    if cs.navdb.wptype[i] in ["VOR", "DME", "TACAN"] and not samedesc:
                        desctxt = desctxt + " " + str(cs.navdb.wpfreq[i]) + " MHz"
                    elif cs.navdb.wptype[i] == "NDB" and not samedesc:
                        desctxt = desctxt + " " + str(cs.navdb.wpfreq[i]) + " kHz"
                ...
                lines += (
                    ...
                    f"Position: {latlon2txt(cs.navdb.wplat[iwp], cs.navdb.wplon[iwp])}\n"
                )
                ...
                # VOR give variation
                if cs.navdb.wptype[iwp] == "VOR":
                    lines += f"Variation: {cs.navdb.wpvar[iwp]} deg\n"

                # How many others?
                n_other = cs.navdb.wpid.count(name) - len(idx_waypoints)
                if n_other > 0:
                    lines += (
                ...
                # In which airways?
                connect = cs.navdb.listconnections(
                    name, cs.navdb.wplat[iwp], cs.navdb.wplon[iwp]
                )
                if len(connect) > 0:
                    awset = set([])
                    for c in connect:
            ...
            # Try airway id
                ...
                awid = name
                airway = cs.navdb.listairway(awid)
                if len(airway) > 0:
                    lines = ""
