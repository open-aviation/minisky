...
import numpy as np
from clearsky.core import TrafficArrays, settings
...
class Trails(TrafficArrays):
    ...
    def create(self, n=1):
        ...
        self.lastlat[-1] = cs.traf.lat[-1]
        self.lastlon[-1] = cs.traf.lon[-1]

    def update(self):
        self.acid = cs.traf.id
        if not self.active:
            self.lastlat = cs.traf.lat
            self.lastlon = cs.traf.lon
            self.lasttim[:] = cs.sim.simt
            return
        ...
        # Check for update
        delta = cs.sim.simt - self.lasttim
        ...
        for i in idxs:
            ...
            lstlat1.append(cs.traf.lat[i])
            lstlon1.append(cs.traf.lon[i])
            lsttime.append(cs.sim.simt)
            ...
            self.lastlat[i] = cs.traf.lat[i]
            self.lastlon[i] = cs.traf.lon[i]
            self.lasttim[i] = cs.sim.simt
        ...
        # Update colours
        self.fcol = (
            1.0 - np.minimum(self.tcol0, np.abs(cs.sim.simt - self.time)) / self.tcol0
        )
