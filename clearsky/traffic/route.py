...
class Route(Replaceable):
    ...
    def addwptMode(acidx, mode=None, value=None):
        ...
        if mode in ["FLYBY", "FLYOVER", "FLYTURN"]:
            ...
            if acrte.swflyby == True and acrte.swflyturn == False:
                cs.scr.echo("Current ADDWPT mode is FLYBY.")
                return True

            elif acrte.swflyby == False and acrte.swflyturn == False:
                cs.scr.echo("Current ADDWPT mode is FLYOVER.")
                return True
    ...
    def addwptStack(acidx, *args):  # args: all arguments of addwpt
        ...
        if len(args) == 1:
            ...
            if swwpmode == "TURNRAD" or swwpmode == "TURNRADIUS":
                ...
                swwpmode == "TURNHDGRATE"
                or swwpmode == "TURNHDG"
                or swwpmode == "TURNHDGR"
            ):
                try:
                    if args[1] == "OFF":
                        acrte.turnhdgr = -999
        ...
        # Take off waypoint: positioned 20% of the runway length after the runway
            ...
            if len(args) == 1 or not args[1]:
                ...
                if rwyrteidx > 0:
                    ...
                    aptidx = cs.navdb.getapinear(rwylat, rwylon)
                    aptname = cs.navdb.aptname[aptidx]

                    rwyname = acrte.wpname[rwyrteidx].split("/")[1]
                    rwyid = rwyname.replace("RWY", "").replace("RW", "")
                    rwyhdg = cs.navdb.rwythresholds[aptname][rwyid][2]

                else:
                    rwylat = cs.traf.lat[acidx]
                    rwylon = cs.traf.lon[acidx]
                    rwyhdg = cs.traf.trk[acidx]
                ...
                rwyid = rwyname.replace("RWY", "").replace(
                    "RW", ""
                )  # take away RW or RWY
                ...
                try:
                    rwyhdg = cs.navdb.rwythresholds[aptid][rwyid][2]
                except:
        ...
        norig = int(cs.traf.ap.orig[acidx] != "")  # 1 if orig is present in route
        ndest = int(cs.traf.ap.dest[acidx] != "")  # 1 if dest is present in route
    ...
    def addwpt_simple(self, iac, name, wptype, lat, lon, alt=-999.0, spd=-999.0):
        ...
        self.addwpt_data(False, self.nwp, newname, wplat, wplon, wptype, alt, spd)
        ...
        if idx >= 0:
            cs.traf.actwp.next_qdr[iac] = self.getnextqdr()
            cs.traf.actwp.swlastwp[iac] = self.iactwp == self.nwp - 1
                    ...
                    if not (cmd in cs.traf.id):
                        ...
                        try:
                            ...
                            if (
                                ...
                                and argtypes[0] == "acid"
                                and not (
                                    len(args) > 2 and args[2].upper() in cs.traf.id
                                )
                            ):
                                # missing acid, so add ownship acid
                                acrte.wpstack[wpidx].append(
                                    acid + " " + " ".join(args[1:])
                                )
                            else:
    ...
    def overwrite_wpt_data(self, wpidx, wpname, wplat, wplon, wptype, wpalt, wpspd):
        """
        Overwrites information for a waypoint, via addwpt_data/9
        """

        self.addwpt_data(True, wpidx, wpname, wplat, wplon, wptype, wpalt, wpspd)
    ...
    def insert_wpt_data(self, wpidx, wpname, wplat, wplon, wptype, wpalt, wpspd):
        ...
        self.addwpt_data(False, wpidx, wpname, wplat, wplon, wptype, wpalt, wpspd)
    ...
    def addwpt(
        ...
        if wptype == Route.orig or wptype == Route.dest:
            ...
            if not name == cs.traf.id[iac] + suffix:  # published identifier
                i = cs.navdb.getaptidx(name)
                if i >= 0:
                    wplat = cs.navdb.aptlat[i]
                    wplon = cs.navdb.aptlon[i]
        ...
        # NORMAL: Wptype is normal waypoint? (lat/lon or nav)
            ...
            # Else make data complete with nav database and closest to given lat,lon
                ...
                if not wptype == Route.runway:
                    i = cs.navdb.getwpidx(name, lat, lon)
                    wpok = i >= 0

                    if wpok:
                        wplat = cs.navdb.wplat[i]
                        wplon = cs.navdb.wplon[i]
                    else:
                        i = cs.navdb.getaptidx(name)
                        wpok = i >= 0
                        if wpok:
    ...
    def direct(acidx: "acid", wpname: "wpinroute"):
        ...
        acrte.iactwp = wpidx
        cs.traf.actwp.lat[acidx] = acrte.wplat[wpidx]
        cs.traf.actwp.lon[acidx] = acrte.wplon[wpidx]
        cs.traf.actwp.flyby[acidx] = acrte.wpflyby[wpidx]
        cs.traf.actwp.flyturn[acidx] = acrte.wpflyturn[wpidx]
        cs.traf.actwp.turnrad[acidx] = acrte.wpturnrad[wpidx]
        cs.traf.actwp.turnspd[acidx] = acrte.wpturnspd[wpidx]
        cs.traf.actwp.turnhdgr[acidx] = acrte.wpturnhdgr[wpidx]

        (
            cs.traf.actwp.nextturnlat[acidx],
            cs.traf.actwp.nextturnlon[acidx],
            cs.traf.actwp.nextturnspd[acidx],
            cs.traf.actwp.nextturnrad[acidx],
            cs.traf.actwp.nextturnhdgr[acidx],
            cs.traf.actwp.nextturnidx[acidx],
        ) = acrte.getnextturnwp()
        ...
        cs.traf.actwp.xtoalt[acidx] = acrte.wpxtoalt[wpidx]
        cs.traf.actwp.nextaltco[acidx] = acrte.wptoalt[wpidx]

        cs.traf.actwp.torta[acidx] = acrte.wptorta[
            wpidx
        ]  # available for active RTA-guidance
        cs.traf.actwp.xtorta[acidx] = acrte.wpxtorta[
            wpidx
        ]  # available for active RTA-guidance

        # VNAV calculations like V/S and speed for RTA
        cs.traf.ap.ComputeVNAV(
            acidx,
            acrte.wptoalt[wpidx],
        ...
        if acrte.wpspd[wpidx] > 0.0:
            ...
            cs.traf.actwp.nextspd[acidx] = cas

        # No speed specified for next leg
        else:
            cs.traf.actwp.nextspd[acidx] = -999.0

        qdr_, dist_ = geo.qdrdist(
            cs.traf.lat[acidx],
            cs.traf.lon[acidx],
            cs.traf.actwp.lat[acidx],
            cs.traf.actwp.lon[acidx],
        )

        # Save leg length & direction in actwp data
        cs.traf.actwp.curlegdir[acidx] = qdr_  # [deg]
        cs.traf.actwp.curleglen[acidx] = dist_ * nm  # [m]

        if (
            acrte.wpflyturn[wpidx] and acrte.wpturnrad[wpidx] > 0.0
        ):  # turn radius specified
        ...
        if (
            ...
            turnrad = cs.traf.tas[acidx] * 360.0 / (2 * pi * acrte.wpturnhdgr[wpidx])
        ...
        cs.traf.actwp.turndist[acidx] = (
            np.logical_or(
                acrte.wpturnhdgr[wpidx] > 0.0, cs.traf.actwp.flyby[acidx] > 0.5
            )
            * turnrad
            * abs(
                math.tan(
    ...
    def getnextwp(self):
        ...
        if self.flag_landed_runway:
            ...
            wphdg = cs.navdb.rwythresholds[name[:4]][rwykey][2]
    ...
    def delrte(acidx: "acid" = None):
        ...
        cs.traf.swlnav[acidx] = False
        cs.traf.swvnav[acidx] = False
        cs.traf.swvnavspd[acidx] = False
    ...
    def calcfp(
        ...
        self.wpdirfrom = self.nwp * [0.0]  # [deg] Direction of leg laving this waypoint
        self.wpdirto = self.nwp * [
            0.0
        ]  # [deg] Direction of leg ot this waypoint (if it exists)
        self.wpdistto = self.nwp * [0.0]  # [nm] Distance of leg to this waypoint in nm
        self.wpialt = self.nwp * [-1]  # wp index of next alttud constraint
        self.wptoalt = self.nwp * [-999.0]  # [m] next alt contraint
        self.wpxtoalt = self.nwp * [
            1.0
        ...
        self.wptorta = self.nwp * [-999.0]  # [s] next time constraint
        self.wpxtorta = self.nwp * [
            1.0
        ...
        iac = cs.traf.id2idx(self.acid)
        qdr, dist = geo.qdrdist(
            cs.traf.lat[iac], cs.traf.lon[iac], self.wplat[0], self.wplon[0]
        )
    ...
    def findact(self, i):
        ...
        dy = wplat - cs.traf.lat[i]
        dx = (wplon - cs.traf.lon[i]) * cs.traf.coslat[i]
        dist2 = dx * dx + dy * dy
        ...
        if iwpnear + 1 < self.nwp:
            ...
            delhdg = abs(degto180(cs.traf.trk[i] - qdr))

            # we only turn to the first waypoint if we can reach the required
            # heading before reaching the waypoint
            time_turn = (
                max(0.01, cs.traf.tas[i])
                * math.radians(delhdg)
                / (g0 * math.tan(cs.traf.ap.bankdef[i]))
            )
            time_straight = (
                math.sqrt(dist2[iwpnear]) * 60.0 * nm / max(0.01, cs.traf.tas[i])
            )
    ...
    def dumprte(acidx: "acid"):
        ...
        with open(cs.data(cs.settings.log_path) / "routelog.txt", "a") as f:
            f.write("\nRoute " + acid + ":\n")
                ...
                f.write(
                    str(
                        (
                            ...
                            acrte.wpname[j],
                            acrte.wptype[j],
                            round(acrte.wplat[j], 4),
                            round(acrte.wplon[j], 4),
                            int(0.5 + acrte.wpalt[j] / ft),
                            int(0.5 + acrte.wpspd[j] / kts),
                            int(0.5 + acrte.wptoalt[j] / ft),
                            round(acrte.wpxtoalt[j] / nm, 3),
                        )
                    )
                    + "\n"
                )
