...
class Coefficient:
    def __init__(self):
        ...
        self.synodict = {}
        with open(cs.data(cs.settings.perf_path_openap) / "synonym.dat", "r") as f_syno:
        ...
        self.engines_fixwing = pd.read_csv(
            cs.data(cs.settings.perf_path_openap) / "fixwing/engines.csv",
            encoding="utf-8",
        )
        ...
        df = pd.read_csv(
            cs.data(cs.settings.perf_path_openap) / "fixwing/dragpolar.csv",
            index_col="mdl",
        )
    ...
    def _load_all_fixwing_flavor(self):
        ...
        acs = json.load(
            open(cs.data(cs.settings.perf_path_openap) / "fixwing/aircraft.json", "r")
        )
        acs.pop("__comment")
        acs_ = {}
    ...
    def _load_all_rotor_flavor(self):
        # read rotor aircraft
        acs = json.load(
            open(cs.data(cs.settings.perf_path_openap) / "rotor/aircraft.json", "r")
        )
        acs.pop("__comment")
        acs_ = {}
        for mdl, ac in acs.items():
    ...
    def _load_all_fixwing_envelop(self):
        ...
        for mdl, ac in self.acs_fixwing.items():
            fenv = (
                cs.data(cs.settings.perf_path_openap)
                / "fixwing/wrap"
                / (mdl.lower() + ".txt")
            )
