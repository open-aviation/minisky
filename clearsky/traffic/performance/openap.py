...
import numpy as np
from clearsky.tools import aero
from clearsky.tools.aero import fpm, ft, kts
from clearsky.traffic.performance.base import PerfBase
...
class OpenAP(PerfBase):
    ...
    def update(self, dt):
        ...
        self.phase = ph.get(
            self.lifttype, cs.traf.tas, cs.traf.vs, cs.traf.alt, unit="SI"
        )
        ...
        rho = aero.vdensity(cs.traf.alt[idx_fixwing])
        vtas = cs.traf.tas[idx_fixwing]
        ...
        max_thrustratio_fixwing = thrust.compute_max_thr_ratio(
            ...
            cs.traf.tas[idx_fixwing],
            cs.traf.alt[idx_fixwing],
            cs.traf.vs[idx_fixwing],
        ...
        self.thrust[idx_fixwing] = (
            self.drag[idx_fixwing] + self.mass[idx_fixwing] * cs.traf.ax[idx_fixwing]
        )
    ...
    def limits(self, intent_v_tas, intent_vs, intent_h, ax):
        ...
        allow_vs = np.where(
            (self.phase == ph.GD) & (cs.traf.tas < self.vminto), 0, allow_vs
        )  # takeoff aircraft
    ...
    def currentlimits(self, id=None):
        ...
        """
        vtasmin = aero.vcas2tas(self.vmin, cs.traf.alt)

        vtasmax = np.minimum(
            aero.vcas2tas(self.vmax, cs.traf.alt), aero.vmach2tas(self.mmo, cs.traf.alt)
        )
