...
class Windfield:
    ...
    def __init__(self):
        ...
        # Axis
        self.altaxis = arange(0.0, self.altmax + self.altstep, self.altstep)
        self.idxalt = arange(0, len(self.altaxis), 1.0)
        ...
        if windalt is not None and len(windalt) > 1:
            ...
            if len(lat) > 3:
                try:
                    ...
                    altaxis = concatenate((array([0.0]), windalt))
    ...
    def addpoint(self, lat, lon, winddir, windspd, windalt=None):
        ...
        if (
            not (type(windalt) in [ndarray, list]) and windalt == None
        ):  # scalar to array
        ...
        if self.fe is not None and self.fn is not None:
            ...
            if self.winddim == 0:  # None = no wind
                ...
                if self.winddim == 2 or (
                    (type(useralt) not in (list, ndarray)) and useralt == 0.0
                ...
                # 3D interpolation as one or more points contain altitude profile
                    ...
                    idxalt = maximum(
                        0.0, minimum(self.altaxis[-1] - eps, alt) / self.altstep
                    )  # find right index
