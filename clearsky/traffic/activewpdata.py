...
from clearsky.tools.aero import g0, nm
...
class ActiveWaypoint(Entity, replaceable=True):
    def __init__(self):
        ...
        with self.settrafarrays():
            ...
            self.turnhdgr = np.array(
                []
            )  # [deg/s]Flyturn turn heading rate (<0 => not specified)
            ...
            self.swlastwp = np.array(
                [], dtype=bool
            )  # switch indicating this is the last waypoint
            self.curlegdir = np.array(
                []
            )  # [deg] direction to active waypoint upon activation
            self.curleglen = np.array(
                []
            )  # [deg] direction to active waypoint upon activation
    ...
    def create(self, n=1):
        ...
        self.nextturnhdgr[
            -n:
        ] = -999.0  # [deg/s] Next turn WP heading rate (<0 => not specified)
        ...
        self.torta[
            -n:
        ] = -999.0  # [s] Req Time of Arrival (RTA) for next wp (-999. = None)
        ...
        self.curlegdir[
            -n:
        ] = -999.0  # [deg] direction to active waypoint upon activation
    ...
    def reached(self, qdr, dist, flyby, flyturn, turnrad, turnhdgr, swlastwp):
        ...
        turntas = np.where(self.turnspd < 0.0, cs.traf.tas, self.turnspd)
        flybyturndist, turnrad = self.calcturn(
            turntas, cs.traf.ap.bankdef, qdr, next_qdr, turnrad, turnhdgr, flyturn
        )
        ...
        close2wp = (
            dist / (np.maximum(0.0001, np.abs(cs.traf.gs))) < 4.0
        )  # Waypoint is within 4 seconds flight time
        tooclose2turn = close2wp * (
            np.abs(degto180(cs.traf.trk % 360.0 - qdr % 360.0)) > 90.0
        )

        # When too close to waypoint or we have passed the active waypoint, based on leg direction,switch active waypoint
        # was:  away  = np.logical_or(close2wp,swlastwp)*(np.abs(degto180(cs.traf.trk%360. - qdr%360.)) > 90.) # difference large than 90
        awayorpassed = np.logical_or(
            tooclose2turn, np.abs(degto180(qdr - cs.traf.actwp.curlegdir)) > 90.0
        )
        ...
        # Detect indices
        # swreached = np.where(cs.traf.swlnav * np.logical_or(awayorpassed,np.logical_or(dist < self.turndist,circling)))[0]
        swreached = np.where(
            cs.traf.swlnav * np.logical_or(awayorpassed, dist < self.turndist)
        )[0]
    ...
    def calcturn(
        ...
        turndist = np.abs(
            turnrad
            * np.tan(
                np.radians(0.5 * np.abs(degto180(wpqdr % 360.0 - next_wpqdr % 360.0)))
            )
        )
