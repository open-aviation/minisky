...
from clearsky.tools.misc import tim2txt
...
class metric_Area:
    ...
    def FIR_circle(self, fir_number):
        ...
        fir_lat.append(cs.navdb.fir[fir_number][1])
        fir_lon.append(cs.navdb.fir[fir_number][2])
...
class metric_CoCa:
    ...
    def cellPlot(self):
        cell = [floor(x / 12) for x in cs.traf.cell]
    ...
    def AircraftCell(self, cells, time):
        if floor(time) >= self.resettime:
            ...
            # filedata = cs.data(settings.log_path) / "coca_20120727-78am-1hour.npy"
            # self.cellPlot(traf)
        ...
        for i in range(cs.traf.ntraf):
            ...
            if cellN > 0:
                cs.traf.cell = np.append(cs.traf.cell, cellN)
                name = "cell" + str(cellN)

                index = np.where(cs.traf.id[i] == self.cells[name][:, [0]])[0]
                if len(index) != 1:
                    j = 0
                    for j in range(0, len(self.cells[name])):
                        if self.cells[name][j][0] == "":
                            break
                    self.cells[name][j][0] = cs.traf.id[i]
                    self.cells[name][j][1] = time
                    self.cells[name][j][2] = cs.traf.ahdg[i]
                    self.cells[name][j][3] = (
                        eas2tas(cs.traf.selspd[i], cs.traf.selalt[i]) / kts
                    )
                    self.cells[name][j][4] = cs.traf.selvs[i] / fpm
                    self.cells[name][j][5] = time
                if len(index) == 1:
                    createtime = float(self.cells[name][index[0]][5])
                    self.cells[name][index[0]][1] = str(time - createtime)

                # self.newaircraft['callsign'][i] = cs.traf.id[i]
...
class metric_HB:
    ...
    def selectTraffic(self):
        ...
        # RECTANGLE AREA
        # for i in range(0,cs.traf.ntraf):
        #     if nx.pnpoly(cs.traf.lat[i],cs.traf.lon[i],self.selected_area) == 1:
        #         traf_selected_lat = np.append(traf_selected_lat,cs.traf.lat[i])
        #         traf_selected_lon = np.append(traf_selected_lon,cs.traf.lon[i])
        #         traf_selected_alt = np.append(traf_selected_alt,cs.traf.alt[i])
        #         traf_selected_tas = np.append(traf_selected_tas,cs.traf.tas[i])
        #         traf_selected_trk = np.append( traf_selected_trk,cs.traf.trk[i])
        #         traf_selected_ntraf = traf_selected_ntraf + 1

        # CIRCLE AREA (FIR Circle)
        for i in range(0, cs.traf.ntraf):
            dist = latlondist(
                cs.sim.metric.fir_circle_point[0],
                cs.sim.metric.fir_circle_point[1],
                cs.traf.lat[i],
                cs.traf.lon[i],
            )

            if dist / nm < cs.sim.metric.fir_circle_radius:
                traf_selected_lat = np.append(traf_selected_lat, cs.traf.lat[i])
                traf_selected_lon = np.append(traf_selected_lon, cs.traf.lon[i])
                traf_selected_alt = np.append(traf_selected_alt, cs.traf.alt[i])
                traf_selected_tas = np.append(traf_selected_tas, cs.traf.tas[i])
    ...
    def saveData(self):
        ...
        fname = cs.data(settings.log_path) / "Metric-HB" / f"{step}-BlueSky.csv"
...
class Metric:
    ...
    def update(self):
        ...
        if self.metric_number == -1 or cs.traf.ntraf < 1:
            return

        """Update: to call for regular logging & runtime analysis"""
        # Only do something when time is there
        if abs(cs.sim.simt - self.t0) < self.dt:
            return
        self.t0 = cs.sim.simt  # Update time for scheduler
        if self.metricstime == 0:
            self.tbegin = cs.sim.simt
            self.metricstime = 1
            print("METRICS STARTED")
            # FIR_circle(cs.navdb,self.fir_number)
        ...
        if cs.sim.simt >= 0:
            if self.metric_number == 0:
                self.metric[self.metric_number].AircraftCell(
                    self.cells, cs.sim.t - self.tbegin
                )
        ...
        for i in range(0, cs.traf.ntraf):
            if (
                cs.traf.selvs[i] <= 0
                and (cs.traf.selalt[i] / ft) < 750
                and cs.traf.selspd[i] < 300
            ):
                deleteAC.append(cs.traf.id[i])

            elif cs.traf.selvs[i] <= 0 and (cs.traf.selalt[i] / ft) < 10.0:
                deleteAC.append(cs.traf.id[i])

            if cs.traf.selvs[i] <= 0 and cs.traf.selspd[i] < 10.0:
                deleteAC.append(cs.traf.id[i])

        for i in range(0, len(deleteAC)):
            cs.traf.delete(deleteAC[i])

        # Heartbeat for test
        self.write(cs.sim.simt, "NTRAF;" + str(cs.traf.ntraf))
        return
