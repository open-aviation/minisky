    ...
    def __init__(self, name, reflat, reflon):
        ...
        # runway type ? "EHAM/RW06","EHGG/RWY27"
            ...
            try:
                ...
                self.lat, self.lon, self.refhdg = cs.navdb.rwythresholds[aptname][
                    rwyname
                ]
            except KeyError:
                self.error = True
        ...
        # airport?
        elif cs.navdb.aptid.count(name) > 0:
            idx = cs.navdb.aptid.index(name.upper())

            self.lat = cs.navdb.aptlat[idx]
            self.lon = cs.navdb.aptlon[idx]
            self.type = "apt"

        # fix or navaid?
        elif cs.navdb.wpid.count(name) > 0:
            idx = cs.navdb.getwpidx(name, reflat, reflon)
            self.lat = cs.navdb.wplat[idx]
            self.lon = cs.navdb.wplon[idx]
            self.type = "nav"

        # aircraft id?
        elif name in cs.traf.id:
            idx = cs.traf.id2idx(name)
            self.name = ""
            self.type = "latlon"
            self.lat = cs.traf.lat[idx]
            self.lon = cs.traf.lon[idx]
