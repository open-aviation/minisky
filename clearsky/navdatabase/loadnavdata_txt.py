...
def loadnavdata_txt():
    ...
    with open(cs.data(settings.navdata_path) / "nav.dat", "rb") as f:
    ...
    # ----------  Read  fix.dat file ----------
    with open(cs.data(settings.navdata_path) / "fix.dat", "rb") as f:
    ...
    with open(cs.data(settings.navdata_path) / "awy.dat", "rb") as f:
    ...
    with open(cs.data(settings.navdata_path) / "airports.dat", "rb") as f:
        types = {"L": 1, "M": 2, "S": 3}
        for line in f:
            line = line.decode(encoding="ascii", errors="ignore").strip()
            # Skip empty lines or comments
    ...
    # Get fir names
    for filname in (cs.data(settings.navdata_path) / "fir").iterdir():
    ...
    with open(cs.data(settings.navdata_path) / "icao-countries.dat", "rb") as f:
        for line in f:
            line = line.decode(encoding="ascii", errors="ignore").strip()
            # Skip empty lines or comments
            if len(line) == 0 or line[0] == "#":
...
def loadthresholds_txt():
    ...
    rwythresholds = dict()
    curthresholds = None
    zfile = ZipFile(cs.data(settings.navdata_path) / "apt.zip")
