...
// Commands go browser -> Channel service -> Redis `from:<channel>:command`
// -> MiniSky's TANGRAM plugin. Results come back on the console topic.
const send = async (cmd: string) => {
  pushLog("cmd", `> ${cmd}`);
  try {
    await tangramApi.realtime.publish(`${miniskyStore.channel}:command`, {
      command: cmd
    });
  } catch (e) {
    pushLog("err", `${e}`);
